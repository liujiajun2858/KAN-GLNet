...
class ContraNorm(nn.Module):
    ...
    def forward(self, x):
        if self.scale > 0.0:
            ...
            if self.dual_norm:
                sim = nn.functional.softmax(sim, dim=1) + nn.functional.softmax(sim, dim=1)
            else:
...
if __name__ == '__main__':
    block = ContraNorm(dim=4096, scale=0.1, dual_norm=False, pre_norm=False, temp=1.0, learnable=False, positive=False, identity=False)
    input = torch.rand(8, 128, 4096)
    output = block(input)
